    ...
    dark:ring-0 dark:border-1 dark:border-gray-600 ring-gray-300 ring-1
     focus-within:ring-blue-300" action="">
        ...
        <div :class="isType ? 'bg-blue-100 dark:bg-blue-600' : 'bg-gray-3 dark:bg-gray-700'" class=" rounded-md z-99 my-1 mr-1">
            <a :href="`/search/hot?q=${keyword}`" id="search" :onclick="onClick">
              <svg :class="isType ? 'text-blue-500' : ''" class="dark:text-white fill-current mx-4 my-2 cursor-pointer" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 48 48" width="24px" height="24px">
...
export default defineComponent({
  ...
  setup () {
    ...
    const keyword = ref('')
    console.log('路由参数', route.query.q)
    if (route.query.q !== '' && route.query.q !== undefined) {
      keyword.value = route.query.q as string
      store.commit('setKey', keyword.value)
      isType.value = true
    }
