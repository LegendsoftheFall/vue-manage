<template>
    <div class="w-full flex flex-shrink-0 flex-col h-max rounded-lg bg-white dark:bg-gray-800 font-Poppins">
      <!-- Title -->
      <div class="flex items-center px-4 py-2 my-3">
      ...
      <div v-for="tags in trendData" :key="tags.id" class=" w-full h-max px-4">
        <!-- Tag -->
        <a :href="`/n/${tags.id}`" target="_blank" v-if="!isLoading" class="flex rounded-lg justify-between items-center hover:bg-gray-300 dark:hover:bg-gray-700 py-2">
          <span class="text-sm truncate ml-1 text-gray-600 dark:text-gray-200">{{tags.name}}</span>
          <span class="flex py-1 px-2 mr-1 tracking-tight rounded-full bg-slate-200 dark:bg-gray-600 text-xs text-gray-600 dark:text-white font-semibold">
            <p>+</p>
            {{tags.num}}
          </span>
        </a>
      ...
      <div class="w-full h-max px-4">
        <!-- See More -->
        <a href="/explore/tags" class="flex rounded-lg items-center hover:bg-gray-300 dark:hover:bg-gray-700 py-2">
...
export default defineComponent({
  ...
  setup () {
    ...
    onMounted(() => {
      ...
      store.dispatch('fetchTrendingTag').then(() => {
        setTimeout(() => {
