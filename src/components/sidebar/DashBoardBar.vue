<template>
    <div class="flex flex-col w-full">
        <router-link :to="{ name: docker.to}" v-for="docker in dockerList" :key="docker.icon" @click="dockerIndex = docker.index"
        class="w-full px-6 py-4 flex flex-shrink-0 items-center"
        :class="dockerIndex === docker.index ? 'bg-blue-450 text-white' : ''">
            <div class="w-10 flex-shrink-0">
                <div v-html="docker.icon"></div>
            </div>
            <div class="flex-shrink-0">{{docker.text}}</div>
        </router-link>
    </div>
</template>
