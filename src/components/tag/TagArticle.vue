  ...
  <div v-for="article in articleList" :key="article.articleInfo.id"
    class="w-full flex flex-col p-4 items-center h-64 border border-t-0 bg-white">
        ...
        <div class="flex items-center">
            ...
            <button v-if="true" class="rounded-full p-1 hover:bg-gray-400">
            ...
            <button v-if="false" class="rounded-full p-1 bg-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 100 100" width="20px" height="20px">
        ...
        <div class="flex items-center">
            ...
            <p class="text-xs text-gray-800 ml-1">{{article.articleInfo.comments}}</p>
            </a>
        </div>
        </div>
    </div>
...
export default defineComponent({
  ...
  setup () {
    ...
    onMounted(() => {
      store.dispatch('fetchTagArticleByID', currentID)
    })
